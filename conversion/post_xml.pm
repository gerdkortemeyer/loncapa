...
# Removes link/@index
sub remove_link_index {
  my ($root) = @_;
  my @links = $root->getElementsByTagName('link');
  foreach my $link (@links) {
    if (defined $link->getAttribute('index')) {
      $link->removeAttribute('index');
    }
  }
}
...
sub convert_conceptgroup {
  ...
  foreach my $conceptgroup (@conceptgroups) {
    ...
    if (!defined $concept && defined $conceptgroup->getAttribute('name')) {
      ...
      $conceptgroup->setAttribute('concept', $concept);
      $conceptgroup->removeAttribute('name');
    }
    if (defined $concept) {
      $conceptgroup->removeAttribute('concept');
